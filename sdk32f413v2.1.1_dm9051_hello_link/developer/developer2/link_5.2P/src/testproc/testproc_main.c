...
#define netif_set_flags(flags, set_flags)     do { flags = (uint8_t)(flags | (set_flags)); } while(0)
#define netif_clear_flags(flags, clr_flags)   do { flags = (uint8_t)(flags & (uint8_t)(~(clr_flags) & 0xff)); } while(0)
