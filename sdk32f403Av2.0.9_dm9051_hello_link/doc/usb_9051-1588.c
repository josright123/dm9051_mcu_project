...
static struct usb_device *dm_device_init(void)
{
    ...
    for (usb_bus = usb_busses; usb_bus; usb_bus = usb_bus->next)
    {
        for (dev = usb_bus->devices; dev; dev = dev->next)
        {
            ...
            else if ((dev->descriptor.idVendor == VENDOR_ID) &&
                    ...
                    for (m = 0; m < alternate->bNumEndpoints; m++)
                    {
                        ...
                        {                                              // direction IN
                            if ((endpoint->bmAttributes & 0x3) == 0x2) // Bulk
                                ...
                                ep_intr = endpoint->bEndpointAddress;
                        }
                        else
                        { // direction OUT
...
struct usb_device *show_sel(void)
{
    ...
    for (i = 0; i < dev_ctr; i++)
    {
        ...
        else if (dev_mode[i] == 1)
        {
            printf("%d: DM9620\n", i);
        }
        else if (dev_mode[i] == 2)
        {
            printf("%d: DM9621\n", i);
        }
        else // JF_12
...
void show_rx()
{
    ...
    {
        if ((rx_status & 0x20) == 0x20)
        { // RX timestamp available~
            if ((rx_status & 0x04) == 0x04)
            { // RX timestamp 8-byte
                reg = 0x72;
                length = 8;
                dm_rd_spi_mems(usb_handle, reg, length, buf);
        ...
        printf("RX timestamp capture enable, but rx packet no rx timestamp!!! \r\n");
...
/*************************************************************************/
void exit_xx(exit_codex) int exit_codex;
{
    printf(" rx_ctr=%ld\n", rx_pkt_ctr);
    printf("rst_ctr=%d, hd9620=%d, size=%d, 1st_size=%d, bi_size=%d\n",
           rx_rst_ctr,
...
/*      write EEPROM                                                     */
/*************************************************************************/
void wr_eep(reg_no, wdata) unsigned char reg_no;
unsigned int wdata;
{
    unsigned char valx;
    unsigned char reg;
...
/*      write PHY                                                        */
/*************************************************************************/
void wr_phy(page, addr, wdata) unsigned int page;
unsigned int addr;
unsigned int wdata;
{
    // unsigned int  reg;
...
/*      write PHY                                                        */
/*************************************************************************/
void wr_phy_fm_nic(reg_no, wdata) unsigned char reg_no;
unsigned int wdata;
{
    unsigned char valx;
    unsigned char reg;
...
void test_1588_gpio_trigger()
{
   ...
   //Read register 0x68, PTP Clock read data (index 0x0 - index 0x7)
    ...
    dm_wr_reg(usb_handle, reg, length, buf);
        ...
        if (GPIO_01 == 0)
        { // Write register 0x62 to GPIO0 (0x10)
            reg = 0x62;
            length = 1;
            buf[0] = 0x10;
            dm_wr_reg(usb_handle, reg, length, buf);
        ...
        if (GPIO_01 == 1)
        { // Write register 0x62 to GPIO1 (0x20)
            reg = 0x62;
            length = 1;
            buf[0] = 0x20;
            dm_wr_reg(usb_handle, reg, length, buf);
...
void test_tx_con(void)
{
    ...
    for (j = 0; j < cntx; j++)
    {
        ...
        if (tx_3b_max > tx_total)
        { // Stone add for check 0x3B*64 > tx_total
        ...
        tx_add_1:
            ...
            { // Stone add for "end of add packet to send!"
