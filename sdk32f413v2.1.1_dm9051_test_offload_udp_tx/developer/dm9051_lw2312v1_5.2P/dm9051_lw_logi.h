...
#define	function_monitor_rx(hdspc, buffer, len) \
	do { \
	sprint_hex_dump0(hdspc, "dm9 monitor <<rx  ", len, 32, buffer, 0, (len < 70) ? len : 70, TRUE); /*, FALSE, DGROUP_NONE */ \
  /* dm_check_rx(buffer, len); */ \
	} while(0)

#define	function_monitor_rx_all(hdspc, buffer, len) \
	do { \
	sprint_hex_dump0(hdspc, "dm9 monitor <<rx_all  ", len, 32, buffer, 0, len /*38*/, TRUE); /*, TRUE, DGROUP_NONE */ \
  /* dm_check_rx(buffer, len); */ \
	} while(0)

#define function_monitor_tx(hdspc, buffer, len) \
	sprint_hex_dump0(hdspc, "dm9 monitor   tx>>", len, 32, buffer, 0, (len < 72) ? len : 72, TRUE); /*, FALSE, DGROUP_NONE */ \
	dm_check_tx(buffer, len)

#define function_monitor_tx_all(hdspc, buffer, len) \
	sprint_hex_dump0(hdspc, "dm9 monitor   tx_all>>", len, 32, buffer, 0, len /*38*/, TRUE); /*, TRUE, DGROUP_NONE */ \
	/*printf("\r\n");*/ \
	dm_check_tx(buffer, len)
#else
