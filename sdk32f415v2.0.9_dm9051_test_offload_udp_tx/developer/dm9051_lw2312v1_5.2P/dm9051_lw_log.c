...
void sprint_hex_dump0(int head_space, const char *prefix_str,
...
//.printf("sprint_hex_dump0, seg_start %d, total len %u\r\n", seg_start, tlen);
		si = seg_start;
		se = seg_start + len;
		for (i = si; i < se; i += rowsize) {
			...
			if (prefix_str) {
				printf("(%s) %.8x: %s", prefix_str, i, linebuf); //"%s", ec->str
			} else {
			...
			if ((i+rowsize) < se)
				...
				//ptr
					...
					printf("  Len %d", ulen);
					printf("  (udp src port %d -> dst port %d, udp payload len %d)", 
						HTONS(UDPBUF->srcport), HTONS(UDPBUF->destport), HTONS(UDPBUF->udplen) - 8);
					printf("\r\n");
				#endif
				}
				//else
...
#if 0
  if (len > 70) { //add.
    printf("---------------debug_tx, Sending total_tx(%d)\r\n", count);
    function_monitor_tx_all(HEAD_SPC, buf, len);
  }
#endif
}
...
void dm9051_log_dump0(const char *prefix_str, size_t tlen, const void *buf, size_t len)
{
	int rowsize = 32;
	int seg_start = 0;
	sprint_hex_dump0(HEAD_SPC, prefix_str, tlen, rowsize, buf, seg_start, len, TRUE);
}

void dm9051_txlog_monitor_tx(int hdspc, const uint8_t *buffer, size_t len)
{
	function_monitor_tx(hdspc, buffer, len);
}
...
{
	function_monitor_rx_all(hdspc, buffer, len);
}
